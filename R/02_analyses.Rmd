...
data_descriptors_pca <- as_tibble(data_descriptors) %>% 
  select(ECOREGION, area_rel, n_sites_abs, n_surveys_abs, first_year, n_15_abs, n_years_abs, n_percent)
...
res_hcpc <- HCPC(res_pca, nb.clust=3) # Arbitrary selection of the number of cluster 
...
data_descriptors_hcpc <- left_join(data_descriptors, res_hcpc$data.clust) %>% 
  # Add a new cluster for cluster with NA (cluster with no data)
  mutate(clust = as.numeric(as.character(clust)),
         clust = replace(clust, is.na(clust), max(clust, na.rm = TRUE) + 1),
...
ggplot() +
  ...
  geom_sf(data = background_map_border, fill = "#56B4E950", color = "grey30", size = 0.5/.pt) +
  # MEOW
  geom_sf(data = data_descriptors_hcpc, aes(fill = clust)) +
  # Background map
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  guides(colour = guide_legend(title.position = "top", title.hjust = 0.5, override.aes = list(size = 5))) +
  scale_fill_manual(values = c("red", "blue", "green", "orange"), 
                     labels = c("High reef area, high monitoring", 
                                "Medium reef area, good monitoring", 
                                "Low reef area, low monitoring", 
                                "No data")) +
  theme_map() +
...
ggsave(filename = "./../figs/10_multivariate-approach-map.png", height = 6, width = 10)

```

## 6.4 Surveys effort vs reef area
...
ggplot(data = data_descriptors, aes(x = area_rel, y = n_surveys_rel, label = ECOREGION)) +
  geom_point() +
  geom_label() +
...
ggplot(data = data_descriptors_effort, aes(x = area_rel, y = survey_deviation, label = ECOREGION)) +

...
ggsave("../figs/01_figure-1.png", dpi = 600)
...
ggsave(filename = "../figs/01_figure-3.png", height = 4, width = 8, dpi = 600)
...
data_source_label <- tibble(Source = c("Government", "NGO", "Non-tourism private sector", 
                                       "Research institute and university", "Tourism", "Unknown"),
                            x = c(2011, 2003, 2009, 2009, 2015, 1988),
                            y = c(0.9, 0.6, 0.4, 0.22, 0.06, 0.5),
                            group = c(1, 2, 2, 2, 1, 1)) # Group 1 = text white, group 2 = text black
...
ggplot(data = data_source, aes(x = Year, y = Rel, fill = Source, label = Source)) +
  ...
  annotate("segment", x = 2015, xend = 2017, y = 0.1, yend = 0.17, col = "white") +
  annotate("segment", x = 2010, xend = 2010, y = 0.31, yend = 0.36, col = "black") +
  scale_fill_brewer(palette = "RdYlBu") +
  lims(x = c(1980, 2020)) +
  labs(y = "Percentage of surveys", x = "Year") +
  scale_y_continuous(labels = scales::percent)
...
ggsave(filename = "../figs/01_figure-4.png", height = 4, width = 8, dpi = 600)

# 5. Remove useless objects ----

rm(data_method_label, data_source_label)
...
res_pca <- PCA(data_descriptors_pca[,-1], ncp = 2)

# 2. HCPC ----

# 2.1 Make the HCPC --

res_hcpc <- HCPC(res_pca, nb.clust = 4, consol = TRUE)
...
write.csv2(res_hcpc$desc.var$quanti$`1`, "./../figs/02_table-results-hcpc_clust4.csv")
write.csv2(res_hcpc$desc.var$quanti$`2`, "./../figs/02_table-results-hcpc_clust3.csv")
write.csv2(res_hcpc$desc.var$quanti$`3`, "./../figs/02_table-results-hcpc_clust2.csv")
write.csv2(res_hcpc$desc.var$quanti$`4`, "./../figs/02_table-results-hcpc_clust1.csv")
...
data_ind <- left_join(data_hcpc_cluster, data_hcpc_descriptors) %>% 
  ...
  mutate(clust = as.character(clust),
         clust = replace_na(clust, "5"),
         clust = case_when(clust == "1" ~ "4",
                           clust == "3" ~ "2",
                           clust == "2" ~ "3",
                           clust == "4" ~ "1",
                           TRUE ~ clust))
...
legend_cluster <- c("1" = glue("<b>1.</b> Very high temporal, high spatial coverage (<i>n</i> = 2)"),
                    "2" = glue("<b>2.</b> High spatio-temporal coverage (<i>n</i> = 16)"),
                    "3" = glue("<b>3.</b> High spatial, medium temporal coverage (<i>n</i> = 42)"),
                    "4" = glue("<b>4.</b> Low spatio-temporal coverage (<i>n</i> = 34)"),
                    "5" = glue("<b>5.</b> No data (<i>n</i> = 13)"))

# Legend labels for each cluster (plot)
annotation_cluster <- tibble(cluster = as.character(c(1:4)),
                             x = c(6, 5.5, 1.5, -1.5),
                             y = c(4.5, -1, 2, -2),
                             col_cluster = c("white", "black", "black", "black"))
...
plot_grid(plot_var, plot_ind, labels = c("A", "B"))
...
plot_grid(plot_var, plot_ind_col, labels = c("A", "B"))

ggsave(filename = "./../figs/01_figure-5.png", height = 5, width = 10, dpi = 600)
...
ggsave(filename = "./../figs/01_figure-6.png", height = 7, width = 10, dpi = 600)
...
for(i in 1:5){
  ...
  geom_sf(data = background_map_border, fill = "#56B4E950", color = "grey30", size = 0.5/.pt) +
  # MEOW
  geom_sf(data = data_meowreefs, fill = "white") +
  # Clusters
  ...
  ggsave(filename = paste0("./../figs/03_map-cluster-", i,".png"), height = 7, width = 10, dpi = 600)

}

# 5. Remove useless objects ----

rm(data_var, data_ind, data_hcpc_descriptors, data_hcpc_cluster, i, legend_cluster, plot_var, plot_ind, plot_ind_col)
...
ggsave(map_area, file = "../figs/03_map-descriptor-reef-area.png", height = 6, width = 10, dpi = 600)
...
ggsave(map_pixel, file = "../figs/03_map-descriptor-pixel.png", height = 6, width = 10, dpi = 600)
...
ggsave(map_first, file = "../figs/03_map-descriptor-first-year.png", height = 6, width = 10, dpi = 600)
...
ggsave(map_years, file = "../figs/03_map-descriptor-years.png", height = 6, width = 10, dpi = 600)
...
ggsave(map_surveys, file = "../figs/03_map-descriptor-surveys.png", height = 6, width = 10, dpi = 600)
...
data_descriptors_maps <- data_gcrmn_meow %>% 
  ...
  mutate(Method = replace_na(Method, "No data"),
         ...
         Method = fct_relevel(Method, c("No data", "Unknown", "Photo-quadrat", "Video transect", "PIT", "LIT", "Visual census"))) %>% 
...
ggsave(map_methods, file = "../figs/03_map-descriptor-methods.png", height = 6, width = 10, dpi = 600)
...
plot_grid(map_area, map_methods, map_pixel, map_surveys, map_first, map_years, map_methods, 
          nrow = 3, ncol = 2, labels = c("A", "B", "C", "D", "E", "F"))

# 2. Save the map ----

ggsave(file = "../figs/02_supplementary-2.png", height = 12, width = 10)

...
data_gcrmn <- read.csv2(file = "../data/03-merge_all_all_all_benthos_NA.csv") %>% 
  ...
  # Add data sources
  left_join(., read.csv2("../data/05_data-sources.csv", na.strings = c("", "NA")))
...
res_hcpc <- HCPC(res_pca, nb.clust=4) # Arbitrary selection of the number of cluster 
...
ggplot() +
  ...
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  guides(colour = guide_legend(title.position = "top", title.hjust = 0.5, override.aes = list(size = 5))) +
  scale_fill_manual(values = c("red", "blue", "green", "orange", "purple", "grey"), 
                     labels = c("High reef area, high monitoring", 
                                "Medium reef area, good monitoring", 
                                "Low reef area, low monitoring", "bla", "bla",
                                "No data")) +
  theme_map() +
...
A <- res_hcpc$call$X %>% 
  mutate(ind = rownames(.)) %>% 
  as_tibble() %>% 
  select(ind, Dim.1, Dim.2, clust)

plot_ind <- ggplot(data = A, aes(x = Dim.1, y = Dim.2, col = clust)) +
...
ggplot() +
  ...
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  # Set guide colours
  scale_fill_gradientn(colours = palette_fill, limits = c(-6, 10), breaks = c(-6, 0, 5, 10)) +
...
ggplot(data = data_source, aes(x = Year, y = Rel, fill = Source, label = Source)) +
  geom_stream(type = "proportional", show.legend = TRUE, alpha = 0.8) +
  #geom_text(data = data_method_label, aes(x = x, y = y, label = Method), family = font_choose_graph) +
  # Alternative using geom_stream_label but less precise
  #geom_stream_label(type = "proportional", family = font_choose_graph) +
  #scale_fill_manual(values = c("#dadfe1", "#FFC740", "#C20008", "#FF020D", "#13AFEF", "#2abb9b")) +

...
source("functions/theme_map_desc.R")
...
data_gcrmn <- read.csv2(file = "../data/03-merge_all_all_all_benthos_NA.csv") %>% 
  ...
  filter(!(DatasetID == "WIOO2.1" & Location == "Bazaruto Island")) %>% 
  filter(!(DatasetID == "WIOO2.28" & Latitude == -12.16667)) %>% 
  # Add data sources
...
ggplot() +
  ...
  # Country boundaries
  geom_sf(data = data_countries_160, size = 0.2) +
...
plot_duration + plot_year + plot_depth + plot_layout(nrow = 1) + 
...
legend_cluster <- c("1" = glue("<b>1.</b> Very high number of surveys, high temporal extent, high spatial extent (<i>n</i> = ", 
                               as.character(legend_cluster[1,"n"]),")"),
                    "2" = glue("<b>2.</b> High temporal extent, medium spatial extent (<i>n</i> = ", 
                               as.character(legend_cluster[2,"n"]),")"),
                    "3" = glue("<b>3.</b> Low number of surveys, low temporal extent, low spatial extent (<i>n</i> = ", 
...
annotation_cluster <- tibble(cluster = as.character(c(1:3)),
                             x = c(6.5, 5, -1.5),
                             y = c(2.5, -1.5, -2),
                             col_cluster = c("white", "black", "white"))
...
plot_ind_col <- ggplot(data = data_ind, aes(x = Dim.1, y = Dim.2, label = ECO_CODE_X, fill = clust)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_point(show.legend = FALSE, size = 4, shape = 21, col = "black", alpha = 0.9) +
  geom_text_repel(show.legend = FALSE, size = 3, seed = 20, max.overlaps = 5, family = font_choose_graph) +
  labs(x = paste0("Dim 1 (", round(res_pca$eig[1,2], 2), "%)"), 
       y = paste0("Dim 2 (", round(res_pca$eig[2,2], 2), "%)"),
       title = "B") +
  geom_label(data = annotation_cluster, aes(x = x, y = y, label = paste("Cluster", cluster), 
             ...
             label.r = unit(0, "lines"), label.size = 0, show.legend = FALSE, family = font_choose_graph) +
...
ggplot() +
  ...
  geom_hline(yintercept = c(2608958.17028999, 0, -2608958.17028999), linetype = "dashed", col = "black", size = 0.2) +
  # Clusters
  geom_sf(data = data_ind, aes(fill = clust), alpha = 0.9, color = "black", size = 3) +
  # Background map
  geom_sf(data = data_map_160, size = 0.25) +
  # Country boundaries
  geom_sf(data = data_countries_160, size = 0.2) +
...
for(i in 1:4){
  ...
  ggplot() +
    ...
    geom_hline(yintercept = c(2608958.17028999, 0, -2608958.17028999), linetype = "dashed", col = "black", size = 0.2) +
    # MEOW
    geom_sf(data = data_meowreefs, fill = "white", color = "black", size = 3) +
    # Clusters
    geom_sf(data = data_ind %>% filter(clust == as.character(i)), aes(fill = clust), alpha = 0.9) +
    # Background map
    geom_sf(data = data_map_160, size = 0.25) +
    # Country boundaries
    geom_sf(data = data_countries_160, size = 0.2) +
  ...
  mutate(Method = str_split_fixed(Method, ",", 6)[,1],
         ...
         Method = case_when(Method %in% c("line intercept transect", 
                            ...
                            Method %in% c("visual census",
                                          ...
                                          "vis est.") ~ "Visual estimation",
                            Method == "video-transect" ~ "Video transect",
                            TRUE ~ "Unknown")) %>% 
  mutate(Method = fct_relevel(Method, c("Unknown", "Photo-quadrat", "PIT", "LIT", "Video transect", "Visual estimation"))) %>% 
...
data_method_label <- tibble(Method = c("Unknown", "Photo-quadrat", "PIT", "LIT", "Video transect", "Visual estimation"),
                            x = c(1997, 2012, 2003, 1989, 2001, 1990),
...
# 1. Transform CRS for data GCRMN sites ----

data_gcrmn_sites <- data_gcrmn_sites %>% 
  st_transform(., crs = 4326)

# 2. Transform CRS for data MEOW reefs ----
...
# 3. Intersection based on buffer for data_countries ----

data_countries_region <- st_intersection(data_buffer, data_countries)

# 4. Select MEOW for the region ----
...
ggplot() +
  ...
  # MEOW
  geom_sf(data = data_meowreefs_region, fill = col_background_meow) +
  # Background map
  geom_sf(data = data_map_region, size = 0.25) +
  # Country boundaries
  geom_sf(data = data_countries_region, size = 0.2) +
  # GCRMN sites
...
data_countries_region <- st_shift_longitude(data_countries_region)
...
### Legend for InkScape layout

```{r}

# 1. Create the legend ----

legend_regional_maps <- ggplot() +
...
legend_regional_maps <- cowplot::get_legend(legend_regional_maps)

# 3. Save the legend ----

ggsave("../figs/03_regional-map-legend.png", legend_regional_maps, 
       width = 5, height = 1.5, dpi = 600, bg = "transparent")

# 4. Remove useless objects ----

rm(legend_regional_maps)
...
ggplot() +
  ...
  geom_sf(data = data_countries_160, size = 0.2) +
  coord_sf(ylim = c(-5000000, 5000000), expand = FALSE) +
  scale_fill_manual(values = c("#2c7bb6", "#91bfdb", "#fc8d59", "#d73027")) +
  guides(fill = guide_legend(title = "Relative coral reef area (%)")) +
  theme_map_desc()

# 3. Save the map ----

ggsave(file = "../figs/03_map-descriptor-reef-area.png", height = 3, width = 12, dpi = 600)
...
data_source <- data_gcrmn_meow %>% 
  ...
  mutate(Source = replace_na(Source, "Unknown"),
         Source = str_replace_all(Source, "Unknown", "Unknown, no data")) %>% 
  st_transform(crs = crs_selected)
...
map_source <- ggplot() +
  ...
  geom_sf(data = data_countries_160, size = 0.2) +
  coord_sf(ylim = c(-5000000, 5000000), expand = FALSE) +
  scale_fill_manual(values = c("#2c7bb6", "#fc8d59",  "#d73027", "white")) +
  guides(fill = guide_legend(title = "Main stakeholder")) +
  theme_map_desc()

# 3. Save the map ----

ggsave(map_source, file = "../figs/03_map-descriptor-source.png", height = 3, width = 12, dpi = 600)
...
data_descriptors_maps <- data_descriptors %>% 
  mutate(percent_n_cut = cut(n_percent, breaks = c(0, 25, 50, 75, 100), include.lowest = TRUE),
         percent_n_cut = ifelse(n_percent == 0, "Unknown, no data", as.character(percent_n_cut)),
         percent_n_cut = factor(percent_n_cut, levels = c("[0,25]", "(25,50]", "(50,75]", "(75,100]", "Unknown, no data")))
...
map_pixel <- ggplot() +
  ...
  geom_sf(data = data_countries_160, size = 0.2) +
  coord_sf(ylim = c(-5000000, 5000000), expand = FALSE) +
  scale_fill_manual(values = c("#2c7bb6", "#91bfdb", "#fc8d59", "#d73027", "white")) +
  guides(fill = guide_legend(title = "Percentage of pixel with data")) +
  theme_map_desc()

# 3. Save the map ----

ggsave(map_pixel, file = "../figs/03_map-descriptor-pixel.png", height = 3, width = 12, dpi = 600)
...
data_descriptors_maps <- data_descriptors %>% 
  mutate(first_year_cut = case_when(is.na(first_year) ~ "Unknown, no data",
                    ...
                    first_year_cut = factor(first_year_cut, 
                                            levels = c("1980 - 1999", "2000 - 2009", "> 2010", "Unknown, no data")))
...
map_first <- ggplot() +
  ...
  coord_sf(ylim = c(-5000000, 5000000), expand = FALSE) +
  scale_fill_manual(values = c("#2c7bb6", "#fc8d59",  "#d73027", "white")) +
  guides(fill = guide_legend(title = "Year with first monitoring")) +
  theme_map_desc()

# 3. Save the map ----

ggsave(map_first, file = "../figs/03_map-descriptor-first-year.png", height = 3, width = 12, dpi = 600)
...
data_descriptors_maps <- data_descriptors %>% 
    mutate(n_years_abs_cut = case_when(n_years_abs == 0 ~ "Unknown, no data",
                                       n_years_abs > 0 & n_years_abs <= 10 ~ "1 - 10",
                                       n_years_abs > 10 & n_years_abs <= 20 ~ "11 - 20",
                                       n_years_abs > 20 ~ "> 21"),
           n_years_abs_cut = factor(n_years_abs_cut, levels = c("1 - 10", "11 - 20", "> 21", "Unknown, no data")))
...
map_years <- ggplot() +
  ...
  coord_sf(ylim = c(-5000000, 5000000), expand = FALSE) +
  scale_fill_manual(values = c("#2c7bb6", "#fc8d59",  "#d73027", "white")) +
  guides(fill = guide_legend(title = "Number of years with data")) +
  theme_map_desc()

# 3. Save the map ----

ggsave(map_years, file = "../figs/03_map-descriptor-years.png", height = 3, width = 12, dpi = 600)
...
data_descriptors_maps <- data_descriptors %>% 
    mutate(n_surveys_rel_cut = case_when(n_surveys_rel == 0 ~ "Unknown, no data",
                                         n_surveys_rel > 0 & n_surveys_rel <= 0.5 ~ "(0,0.5]",
                                         n_surveys_rel > 0.5 & n_surveys_rel <= 1 ~ "(0.5,1]",
                                         n_surveys_rel > 1 ~ "> 1"),
           n_surveys_rel_cut = factor(n_surveys_rel_cut, levels = c("(0,0.5]", "(0.5,1]", "> 1", "Unknown, no data")))
...
map_surveys <- ggplot() +
  ...
  coord_sf(ylim = c(-5000000, 5000000), expand = FALSE) +
  scale_fill_manual(values = c("#2c7bb6", "#fc8d59", "#d73027", "white")) +
  guides(fill = guide_legend(title = "Relative number of surveys (%)")) +
  theme_map_desc()

# 3. Save the map ----

ggsave(map_surveys, file = "../figs/03_map-descriptor-surveys.png", height = 3, width = 12, dpi = 600)
  ...
  mutate(Method = str_split_fixed(Method, ",", 6)[,1],
         ...
         Method = case_when(Method %in% c("line intercept transect", 
                            ...
                            Method %in% c("visual census",
                                          ...
                                          "vis est.") ~ "Visual estimation",
                            Method == "video-transect" ~ "Video transect",
                            TRUE ~ "Unknown, no data")) %>%
  left_join(data_meowreefs, .) %>% 
  mutate(Method = replace_na(Method, "Unknown, no data"),
         Method = as.factor(Method),
         Method = fct_relevel(Method, c("Photo-quadrat", "Video transect", 
                                        "PIT", "LIT", "Visual estimation", "Unknown, no data"))) %>% 
...
map_methods <- ggplot() +
  ...
  geom_sf(data = data_countries_160, size = 0.2) +
  coord_sf(ylim = c(-5000000, 5000000), expand = FALSE) +
  scale_fill_manual(values = c("#2c7bb6", "#91bfdb", "#f7ca18", "#fc8d59", "#d73027", "white")) +
  guides(fill = guide_legend(title = "Most used method")) +
  theme_map_desc()

# 3. Save the map ----

ggsave(map_methods, file = "../figs/03_map-descriptor-methods.png", height = 3, width = 12, dpi = 600)
...
map_methods + map_source + map_pixel + map_surveys + map_first + map_years + 
  theme(plot.tag = element_text(face = "bold"),
...
ggsave(file = "../figs/02_supplementary-3.png", height = 12, width = 10)
...
plots <- plot_list[["n_sites_abs"]] + 
  plot_list[["n_surveys_abs"]] +
  plot_list[["first_year"]] +
  plot_list[["n_15_abs"]] +
  plot_list[["n_years_abs"]] +
  plot_list[["n_percent"]] &
  theme(legend.position = "top") # Shared legend

plots + plot_layout(ncol = 1, guides = "collect")
...
plot_area + plot_surveys + plot_years + plot_pixel + plot_layout(nrow = 1)
...
## Long-tail of monitoring

```{r}

# 1. Transform the data ----

data_longtail <- data_gcrmn %>% 
  select(Latitude, Longitude, Date, Year, DatasetID) %>% 
  distinct(.) %>% 
  group_by(DatasetID) %>% 
  count() %>% 
  ungroup() %>% 
...
ggplot(data = data_longtail, aes(x = name, y = n)) +
  geom_point() +
  geom_path() +
  lims(y = c(0, 25), x = c(0, 400)) +
  labs(x = "Dataset index", y = "Relative size of the dataset\n(% of observations)") +
...
ggsave(filename = "../figs/03_long-tail-ecological-monitoring.png", dpi = 600, width = 6, height = 4, bg = "transparent")
...
## Relationship between GDP and monitoring effort

```{r}

# 1. Join GDP and EEZ data (attribute a GDP to each EEZ) ----

data_gdp <- read_xls(path = "../data/06_data-world-bank_gdp.xls", skip = 2) %>% 
  select("Country Code", "2018") %>% 
  rename(ISO_SOV1 = "Country Code", gdp = "2018")

data_gdp <- st_read("../data/07_data-eez/eez_v11.shp") %>% 
  left_join(., data_gdp)

# 2. Intersection between MEOW and EEZ ----

data_gdp <- st_intersection(data_gdp, data_meowreefs) %>% 
  mutate(area = st_area(geometry),
         area = as.numeric(area)) %>% 
  st_drop_geometry() %>% 
  group_by(ECOREGION) %>% 
  mutate(total_area = sum(area),
         area_rel = (area*100)/total_area) %>% 
  ungroup() %>% 
  mutate(gdp = gdp*area_rel) %>% 
  group_by(ECOREGION) %>% 
  summarise(gdp = mean(gdp, na.rm = TRUE))

# 3. Make the plot ----

data_descriptors <- read.csv2("../figs/02_table-of-descriptors.csv")

data_gdp <- data_gdp %>% 
  rename(Name = ECOREGION) %>% 
  left_join(., data_descriptors)

ggplot(data = data_gdp, aes(x = gdp, y = Surveys, label = Name)) +
  geom_point() +
  geom_label(size = 2.5) +
  labs(x = "GDP (constant $US)", y = "Number of surveys")

# 4. Save the plot ----

ggsave("../figs/03_relationship-gdp-surveys.png", dpi = 600, width = 7, height = 5)

```

## GCRMN consortium list

```{r eval=FALSE, include=FALSE}

# 1. Import the list of people who contributes for the 2020 GCRMN report ----

list_consortium <- read.csv2("../misc/gcrmn-consortium_list-gcrmn-2020.csv", encoding = "latin1") %>% 
  select(-Area)

# 2. Export the list of people for which data were used in this article ----

data_gcrmn %>% 
  select(DatasetID) %>% 
  distinct() %>% 
  left_join(., list_consortium) %>% 
  janitor::clean_names() %>% 
  rename(owner_name = name, owner_email = email) %>% 
  mutate(owner_affiliation = NA, .before = owner_email) %>% 
  mutate(mail_language = NA, first_mail = NA, second_mail = NA, reply = NA) %>% 
  write.csv2(., file = "../misc/gcrmn-consortium_list-authors.csv", row.names = FALSE, fileEncoding = "latin1")

...
plot_grid(plot_area, plot_surveys, plot_years, nrow = 1, rel_widths = c(1, 0.425, 0.425))

# 4. Save the plot ----

ggsave(filename = "../figs/07_ecoregions-data-descriptors.png", height = 15, width = 5)

# 5. Remove useless objects ----

rm(plot_area, plot_surveys, plot_years)
...
# 2. Create the grid of 5° ----

AA <- st_make_grid(data_reefs, cellsize = 1) %>% 
  st_as_sf()

# 3. Test if the grid is correct ----

ggplot() +
  geom_sf(data = AA) +
  geom_sf(data = data_reefs)

# 5. Join to get grid cells containing coral reefs ----

BB <- st_join(AA, data_reefs)

# 6. Plot ----

ggplot() +
  #geom_sf(data = AAA) +
  geom_sf(data = BB, aes(fill = GRIDCODE)) +
...
AA <- st_make_grid(data_reefs, cellsize = 0.25)

ggplot() +
  geom_sf(data = AA) +
  geom_sf(data = data_reefs) +
  coord_sf(xlim = c(40, 45.5), ylim = c(-12, -13.5)) 

...
annotation_label <- tibble(descriptor = data_var$var,
                           x = c(0.75, 0.625, -0.625, 0.4, 0.70, 0.275),
                           y = c(0.08, 0.335, 0.27, 0.6, -0.35, -0.725))

# 1.2 Make the graph --

plot_var <- ggplot(data = data_var) +
  #geom_text_repel(aes(x = Dim.1, y = Dim.2, label = var), family = font_choose_graph) +
  geom_text(data = annotation_label, aes(x = x, y = y, label = descriptor)) +
...
legend_cluster <- c("1" = glue("<b>1.</b> Very high spatio-temporal coverage (<i>n</i> = ", as.character(legend_cluster[1,"n"]),")"),
                    "2" = glue("<b>2.</b> High spatio-temporal coverage (<i>n</i> = ", as.character(legend_cluster[2,"n"]),")"),
                    "3" = glue("<b>3.</b> Medium spatio-temporal coverage (<i>n</i> = ", as.character(legend_cluster[3,"n"]),")"),
                    "4" = glue("<b>4.</b> Low spatio-temporal coverage (<i>n</i> = ", as.character(legend_cluster[4,"n"]),")"),
                    "5" = glue("<b>5.</b> No data (<i>n</i> = ", as.character(legend_cluster[5,"n"]),")"))
...
ggsave(file = "../figs/03_map-descriptor-reef-area.png", height = 6, width = 10, dpi = 600)
...
map_source <- ggplot() +
  ...
  # Background map
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  scale_fill_manual(values = c("#2c7bb6", "#fc8d59",  "#f7ca18", "#ecf0f1")) +
  guides(fill = guide_legend(title.position = "top",
                             title.hjust = 0.5, nrow = 2,
                             title = "Main stakeholder")) +
  labs(title = "MAIN STAKEHOLDER") + 
  theme_map2()

# 3. Save the map ----

ggsave(map_source, file = "../figs/03_map-descriptor-source.png", height = 6, width = 10, dpi = 600)
...
map_pixel <- ggplot() +
  ...
  # Background map
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  scale_fill_manual(values = c("#ecf0f1", "#fc8d59", "#f7ca18", "#91bfdb", "#2c7bb6")) +
  guides(fill = guide_legend(title.position = "top",
                             title.hjust = 0.5,
...
map_first <- ggplot() +
  ...
  # Background map
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  scale_fill_manual(values = c("#2c7bb6",  "#91bfdb", "#fc8d59", "#ecf0f1")) +
  guides(fill = guide_legend(title.position = "top",
                             title.hjust = 0.5,
...
map_years <- ggplot() +
  ...
  # Background map
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  scale_fill_manual(values = c("#ecf0f1", "#fc8d59", "#f7ca18", "#2c7bb6")) +
  guides(fill = guide_legend(title.position = "top",
                             title.hjust = 0.5,
...
for(i in i_colnames){
  ...
  plot_i <- ggplot(data = data_descriptors_clust, aes_string(x = i, y = "y", label = "ECO_CODE_X")) +
    ...
    scale_fill_manual(values = palette_cluster, name = "Cluster") +

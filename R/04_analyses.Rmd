...
plot_grid(plot_area, plot_surveys, plot_years, plot_pixel, nrow = 1, rel_widths = c(1, 0.36, 0.36, 0.36))

# 4. Save the plot ----

ggsave(filename = "../figs/08_ecoregions-data-descriptors.png", height = 15, width = 14)
...
# 1. Transform n_surveys in relative ----

data_descriptors <- data_descriptors %>% 
  mutate(survey_total = sum(n_surveys)) %>% 
  group_by(ECOREGION) %>% 
  mutate(surveys_rel = (n_surveys*100)/survey_total) %>% 
  select(-survey_total)

# 2. Select MEOW to label ----

data_descriptors <- data_descriptors %>% 
  mutate(labelisation = case_when(area_rel > 4 ~ glue("{ECOREGION} (<b style='color:{color}'>{ECO_CODE_X}</b>)"),
                                  TRUE ~ NA_character_))
...
AA <- ggplot(data = data_descriptors, aes(x = area_rel, y = surveys_rel, label = labelisation)) +
  geom_point(shape = 21, fill = col_color_graph, size = 2) +
  geom_richtext() +
  geom_abline(intercept = 0) +
  labs(x = "Relative reef surface (%)", y = "Relative surveys number (%)") +
  lims(x = c(0, 12.5), y = c(0, 12.5))
...
ggplot(data = data_descriptors, aes(x = area_rel, y = surveys_rel)) +
  geom_point(shape = 21, fill = col_color_graph, size = 2) +
  geom_abline(intercept = 0) +
  labs(x = "Relative reef surface (%)", y = "Relative surveys number (%)") +
  lims(x = c(0, 12.5), y = c(0, 12.5))
...
PCA(data_descriptors[,c(2, 4:10, 13)])
PCA(data_descriptors[,c(4:10, 13)])

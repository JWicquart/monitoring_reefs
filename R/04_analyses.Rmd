...
# 8. Add reef surfaces and calculate relative surfaces ----
...
data_descriptors <- left_join(data_surface, data_descriptors) %>% 
  ...
  group_by(ECOREGION) %>% 
  mutate(area_rel = (area*100)/area_total) %>% 
  ungroup() %>% 
  select(-area_total)

# 9. Add human population density and GDP ----

load("./../data/04_meow-with-coral-reefs-gdp.RData")

load("./../data/04_meow-population.RData")

data_descriptors <- left_join(data_meowpop, data_descriptors) %>% 
  left_join(., data_meowgdp) %>% 
  mutate_at(5:13, ~replace_na(., 0))

# 10. Remove useless objects ----

rm(data_meowgdp, data_meowpop, data_surface)
...
data_descriptors <- left_join(pixel_total, pixel_monitoring) %>% 
  ...
  left_join(data_descriptors, .) %>% 
  left_join(as.tibble(data_meowreefs), .) %>% 
  st_as_sf()

# 7. Remove useless objects ----

rm(data_reefs, grid_reefs, pixel_total, pixel_monitoring)
...
ggpairs(data_descriptors[,15:30])

# 2. Save the plot ----

ggsave("./../figs/08_pairs-of-data-descriptors.png", height = 20, width = 20)
...
plot_surveys <- ggplot(data = data_descriptors, aes(x = fct_reorder(name, area_rel), y = n_surveys)) +
  geom_point() +
  geom_segment(aes(xend = fct_reorder(name, area_rel), y = 0, yend = n_surveys)) +
  coord_flip() +
  theme(axis.text.y = element_markdown()) +
  labs(x = NULL, y = "Number of surveys") +
...
plot_years <- ggplot(data = data_descriptors, aes(x = fct_reorder(name, area_rel), y = n_years)) +
  geom_point() +
  geom_segment(aes(xend = fct_reorder(name, area_rel), y = 0, yend = n_years)) +
  coord_flip() +
  theme(axis.text.y = element_markdown()) +
  labs(x = NULL) +
  theme(axis.title.y = element_blank(),
...
## 6.4 Effort vs surface

### 6.4.1 Surveys

```{r}

data_descriptors2 <- data_descriptors %>% 
  mutate(total_surveys = sum(n_surveys)) %>% 
  group_by(ECOREGION) %>% 
  mutate(surveys_rel = (n_surveys*100)/total_surveys) %>% 
  ungroup() %>% 
  select(-total_surveys) %>% 
  mutate(survey_deviation = surveys_rel - area_rel) %>% 
  st_as_sf()

# 1. Make the plot ----

ggplot(data = data_descriptors2, aes(x = area_rel, y = surveys_rel, label = ECOREGION)) +
  geom_point() +
  geom_label() +
...
ggplot(data = data_descriptors2, aes(x = area_rel, y = survey_deviation, label = ECOREGION)) +
  geom_segment(aes(x = area_rel, xend = area_rel, y = 0, yend = survey_deviation)) +
  geom_point(shape = 21, col = "white", fill = "black", size = 3) +
...
palette_fill <- c("#2c82c9", "white", "#cf000f")

ggplot() +
  # Background map border
  geom_sf(data = background_map_border, fill = "#56B4E950", color = "grey30", size = 0.5/.pt) +
  # MEOW
  geom_sf(data = data_descriptors2, aes(fill = survey_deviation)) +
  # Background map
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  # Set guide colours
  scale_fill_gradientn(colours = palette_fill, limits = c(-6, 6), breaks = c(-6, -4, -2, 0, 2, 4, 6)) +
...
quantile(data_descriptors2$n_percent)

# 1. Make the plot ----

ggplot(data = data_descriptors2, aes(x = area_rel, y = n_percent, label = ECOREGION)) +
  geom_point() +
  geom_label() +
  geom_abline(intercept = 0) +
  labs(x = "Relative reef surface (%)", y = "Relative sites number (%)")
...
ggplot() +
  ...
  geom_sf(data = background_map_border, fill = "#56B4E950", color = "grey30", size = 0.5/.pt) +
  # MEOW
  geom_sf(data = data_descriptors2, aes(fill = n_percent)) +
  # Background map
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  # Set guide colours
  scale_fill_gradientn(colours = palette_fill, limits = c(0, 100), breaks = c(0, 25, 50, 75, 100)) +
  guides(fill = guide_colourbar(title = "Deviation from expected",
                                title.position = "top",
                                barwidth = 18,
                                barheight = 1,
...
## 6.5 Maps of descriptors

### 6.5.1 Relative coral reef area
...
ggplot() +
  ...
  geom_sf(data = background_map_border, fill = "#56B4E950", color = "grey30", size = 0.5/.pt) +
  # MEOW
  geom_sf(data = data_descriptors, aes(fill = area_rel)) +
  # Background map
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  # Set guide colours
  scale_fill_gradientn(colours = palette_fill, limits = c(0, 7.002), breaks = c(0, 1, 2, 3, 4, 5, 6, 7)) +
  guides(fill = guide_colourbar(title = "Relative coral reef area (%)",
                                title.position = "top",
                                barwidth = 18,
                                barheight = 1,
                                frame.colour = "black",
...
ggsave("../figs/01_global-map-relative-reef-area.png", height = 6, width = 10)

```

### 6.5.2 Pixel with data
...
ggplot() +
  ...
  geom_sf(data = background_map_border, fill = "#56B4E950", color = "grey30", size = 0.5/.pt) +
  # MEOW
  geom_sf(data = data_descriptors, aes(fill = n_percent)) +
  # Background map
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  # Set guide colours
  scale_fill_gradientn(colours = palette_fill, limits = c(0, 100), breaks = c(0, 25, 50, 75, 100)) +
  guides(fill = guide_colourbar(title = "Percentage of pixel with data",
                                title.position = "top",
                                barwidth = 18,
                                barheight = 1,
                                frame.colour = "black",
...
ggsave("../figs/01_global-map-relative-pixel-with-data.png", height = 6, width = 10)
...
test <- as.tibble(data_descriptors)

res_pca <- PCA(test[,c(15, 16, 23, 26, 27)])
...
ggsave(filename = "./../figs/10_pca-data-descriptors.png")

```

```{r}

AA <- HCPC(res_pca, nb.clust=4, iter.max = 100)

BB <- left_join(data_descriptors, AA$data.clust) %>% 
  select(-color, -name) %>% 
  left_join(as.tibble(data_meowreefs), .) %>% 
  st_as_sf()
...
ggplot() +
  ...
  geom_sf(data = background_map_border, fill = "#56B4E950", color = "grey30", size = 0.5/.pt) +
  # MEOW
  geom_sf(data = BB, aes(fill = clust)) +
  # Background map
  geom_sf(data = data_map, fill = col_fill_map, col = col_color_map) +
  guides(colour = guide_legend(title.position = "top", title.hjust = 0.5, override.aes = list(size = 5))) +
  scale_color_manual(values = palette_duration) +
